...
export default function create(store, option) {
  ...
  if (arguments.length === 2) {
    ...
    getApp().globalData && (getApp().globalData.store = store); //option.data = store.data
    ...
    option.onLoad = function (e) {
      ...
      store.custom && extendsCustom(store.custom, this);
      rewriteUpdate(this);
    ...
    store.ready = function () {
      if (pure) {
        ...
        this.store.custom && extendsCustom(this.store.custom, this);
...
function initCloud(env) {
  tt.cloud.init();
  globalStore.db = tt.cloud.database({
    env: env
  });
}
...
} // 借用westore 挂载自定义属性到 this 上  如api utils


function extendsCustom(custom, ctx) {
  if (type(custom) !== '[object Object]') return console.warn('["custom"]  must be an object in store');
  Object.keys(custom).forEach(key => {
    if (ctx.hasOwnProperty(key)) {
      console.warn(`${key} has been defined`);
    } else {
      ctx[key] = custom[key];
    }
  });
