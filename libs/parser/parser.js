...
Component({
  ...
  detached() {
    ...
    this.imgList.each(src => {
      if (src && src.includes(tt.env.USER_DATA_PATH) && fs) fs.unlink({
        filePath: src
      });
    });
  ...
  methods: {
    ...
    getText(ns = this.data.nodes) {
      ...
      for (var i = 0, n; n = ns[i++];) {
        if (n.type == 'text') txt += n.text.replace(/&nbsp;/g, '\u00A0').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');else if (n.type == 'br') txt += '\n';else {
          // 块级标签前后加换行
    ...
    setContent(html, append) {
      var nodes,
          parser = new Parser(html, this.data); // 缓存读取
      ...
      var data = {};
      if (append) for (let i = this.data.nodes.length, j = nodes.length; j--;) data[`nodes[${i + j}]`] = nodes[j];else data.nodes = nodes;
      if (this.showWithAnimation) data.showAm = 'animation: show .5s';
      this.setData(data, () => {
        this.triggerEvent('load');
      }); // 设置标题

      if (nodes.title && this.data.autosetTitle) tt.setNavigationBarTitle({
        title: nodes.title
      });
      ...
      for (let i = 0, n; n = ns[i++];) {
        ...
        for (let j = 0, item; item = n.data.nodes[j++];) {
          if (item.c) continue; // 获取图片列表

          if (item.name == 'img') this.imgList.setItem(item.attrs.i, item.attrs['original-src'] || item.attrs.src); // 音视频控制
          else if (item.name == 'video' || item.name == 'audio') {
              var ctx;
              if (item.name == 'video') ctx = tt.createVideoContext(item.attrs.id, n);else ctx = n.selectComponent('#' + item.attrs.id);
