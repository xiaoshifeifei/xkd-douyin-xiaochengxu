...
Component({
  ...
  methods: {
    ...
    play(e) {
      ...
      if (this.top.videoContexts.length > 1 && this.top.data.autopause) for (var i = this.top.videoContexts.length; i--;) if (this.top.videoContexts[i].id != e.currentTarget.id) this.top.videoContexts[i].pause();
    },
    ...
    loadImg(e) {
      ...
      if (this.data.lazyLoad && !this.data.ctrl[i]) this.setData({
        [`ctrl[${i}]`]: 1
      });else if (this.data.loading && this.data.ctrl[i] != 2) this.setData({
        [`ctrl[${i}]`]: 2
      });
    },
    ...
    linkpress(e) {
      ...
      if (jump) {
        if (attrs['app-id']) tt.navigateToMiniProgram({
          appId: attrs['app-id'],
          path: attrs.path
        });else if (attrs.href) {
          if (attrs.href[0] == '#') this.top.navigateTo({
            id: attrs.href.substring(1)
          });else if (attrs.href.indexOf('http') == 0 || attrs.href.indexOf('//') == 0) tt.setClipboardData({
            data: attrs.href,
            success: () => tt.showToast({
              title: '链接已复制'
            })
          });else tt.navigateTo({
            url: attrs.href,
    ...
    error(e) {
      ...
      if (source == 'video' || source == 'audio') {
        ...
        if (index < node.attrs.source.length) return this.setData({
          [`nodes[${i}].i`]: index
        });
